...
bool RgbDmaBufGl::initialize()
{
    ...
    if (programId_ == 0) {
        lastInitFailure_ = QStringLiteral("RGB shader init failed (%1)").arg(shaderFailures.join(QStringLiteral("; ")));
        return false;
    }
...
void RgbDmaBufGl::draw(const QSize &widgetSize, const QRect &targetRect, const float devicePixelRatio)
{
    ...
    glUseProgram(programId_);
    if (boundPixelWidth_ != lastSentPixelWidth_) {
        glUniform1f(pixelWidthUniform_, boundPixelWidth_);
        lastSentPixelWidth_ = boundPixelWidth_;
    }
    if (boundByteWidth_ != lastSentByteWidth_) {
        glUniform1f(byteWidthUniform_, boundByteWidth_);
        lastSentByteWidth_ = boundByteWidth_;
    }
    const auto bgrInt = boundBgr_ ? 1 : 0;
    if (bgrInt != lastSentBgr_) {
        glUniform1i(bgrUniform_, bgrInt);
        lastSentBgr_ = bgrInt;
    }
    const auto flipInt = boundFlipVertical_ ? 1 : 0;
    if (flipInt != lastSentFlipY_) {
        glUniform1i(flipUniform_, flipInt);
        lastSentFlipY_ = flipInt;
    }
